...
namespace Topology.IO.Dwg.CS
{
    ...
    public class GraphBuilder2
    {
        ...
        public readonly AdjacencyGraph<Coordinate, IEdge<Coordinate>> graph;
            ...
            foreach (var line in strings)
            {
                ...
                if (bound > 0)
                {
                    for (int counter = 0; counter < bound; counter++)
                    {
                        ...
                        var lineString = factory.CreateLineString(new[] { src, dst, });
        ...
        public LineString Perform(Coordinate source, Coordinate destination)
        {
            ...
            // Build algorithm
            var dijkstra = new DijkstraShortestPathAlgorithm<Coordinate, IEdge<Coordinate>>(graph, edge => consts[edge]);

            // Attach a Distance observer to give us the distances between edges
            var distanceObserver = new VertexDistanceRecorderObserver<Coordinate, IEdge<Coordinate>>(edge => consts[edge]);
            distanceObserver.Attach(dijkstra);

            // Attach a Vertex Predecessor Recorder Observer to give us the paths
            var predecessorObserver = new VertexPredecessorRecorderObserver<Coordinate, IEdge<Coordinate>>();
            ...
            bool result = predecessorObserver.TryGetPath(destination, out var path);

            // Then we need to turn that into a geomery.
            return result ? BuildString(new List<IEdge<Coordinate>>(path)) : null;

            // if the count is greater than one then a path could not be found, so we return null
        }
